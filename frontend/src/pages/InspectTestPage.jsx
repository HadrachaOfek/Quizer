  ...
  return (
    <DefaultBackground>
      <Stack width="99vw" boxSizing="border-box" padding="20px">
        ...
        {data && (
          <Stack rowGap={2} sx={{ marginTop: "2rem" }} component={Container}>
            <Card component={Card} variant="outlined">
              <CardContent
                sx={{
                  display: "grid",
                  gap: "2rem",
            ...
            {options
              ...
              .questions.map(({ answers, linkedQuestion }, index) => {
                console.log(answers, questions.get(linkedQuestion).question);
                return (
                  <QuestionCard
                    readOnly={true}
                    selected={answers}
                    key={linkedQuestion + "" + index}
                    id={linkedQuestion}
                    answers={questions.get(linkedQuestion).answers}
                    correctAns={questions.get(linkedQuestion).correctAnswers}
                    question={questions.get(linkedQuestion).question}
                    type={questions.get(linkedQuestion).type}
