	...
	useEffect(() => {
		const getData = async () => {
			const res = await axios.get(
				ServerAddress(`user/connect/${id}/${password}`)
			);
				...
				<Fab
					...
					onClick={e =>
						window.location.replace(
							`/dashboard/create_test/${userData.userId}/${userData.password}`
						)
					}>
					<AddIcon />
				</Fab>
			...
			<Grid container sx={{ padding: '20px',boxSizing : 'border-box' , width : '95vw' , margin : 'auto' }} spacing={1}>
				...
				<Grid item xs={8}>
					<Box
						...
						/>
						<Typography variant='h4'>מערכת ניהול בחנים</Typography>
					</Box>
				</Grid>
				<Grid item xs={2}>
					<img
...
const TableHeader = () => {
	return (
		<React.Fragment>
			...
			<Grid item xs={3}>
				<Typography variant='caption'>שם המבחן</Typography>
			</Grid>
			<Grid item xs={2}>
				<Typography variant='caption'>מספר הגשות</Typography>
			</Grid>
			<Grid item xs={3}>
				<Typography variant='caption'>נפתח לאחרונה</Typography>
			</Grid>
			<Grid item xs={2}>
				<Typography variant='caption'>ממוצע ציונים</Typography>
			</Grid>
			<Grid item xs={2}>
				<Typography variant='caption'>פעולות</Typography>
			</Grid>
		</React.Fragment>
	);
};
