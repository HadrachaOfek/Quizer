...
function TestsTable({
  ...
  const activateTest = async (testId) => {
    const res = await axios.patch(
      ServerAddress(`test/activate_test/${userId}/${password}/${testId}`)
    );
    if (res.data[0]) {
  ...
  const deleteTest = async (testId) => {
    const res = await axios.delete(
      ServerAddress(`test/delete_test/${userId}/${password}/${testId}`)
    );
    if (res.data[0]) {
      setSnackbarMessage(res.data[1]);
      setSnackbarSeverity("success");
  ...
  const deactivateTest = async (testId) => {
    const res = await axios.patch(
      ServerAddress(`test/deactivate_test/${userId}/${password}/${testId}`)
    );
    if (res.data[0]) {
      setSnackbarMessage(res.data[1]);
      setSnackbarSeverity("success");
  ...
  const editTest = (testid) => {
    window.location.replace(
      `/dashboard/edit_questions/${userId}/${password}/${testid}`
    );
  };

  const editUsers = (testid) => {
    window.location.replace(
      `/dashboard/edit_users/${userId}/${password}/${testid}`
    );
  };
...
const TestsTableRow = ({
  ...
  return (
    <TableRow>
      ...
      <TableCell>
        <CircleIcon htmlColor={active ? "green" : "red"} />
      </TableCell>
      ...
      <TableCell>
        ...
        <Tooltip
          title={active ? "כבה" : "הדלק"}
          onClick={(e) => (active ? deactivateTest(_id) : activateTest(_id))}
        >
          <IconButton color="primary">
        ...
        <Tooltip title="רשימת נבחנים" onClick={(e) => editUsers(_id)}>
          <IconButton color="primary">
            <GroupAdd />
          </IconButton>
        </Tooltip>
